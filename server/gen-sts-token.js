...
async function run() {
    try {
        ...
        const response = await sts.send(new GetSessionTokenCommand({ DurationSeconds: 43200 }));
        
        const accessKey = response.Credentials.AccessKeyId;
        const secretKey = response.Credentials.SecretAccessKey;
        ...
        const finalToken = 'bedrock-api-key-' + Buffer.from(signedUrl).toString('base64');
